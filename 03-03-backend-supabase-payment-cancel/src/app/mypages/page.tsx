...
export default function GlossaryMagazinesMypage() {
  ...
  const { handleCancelSubscription, isLoading } = usePaymentCancel();

  // 구독 정보 조회
  useEffect(() => {
    const fetchSubscription = async () => {
      try {
        // 현재 구독 중인 결제 정보 조회 (status가 'Paid'이고 가장 최근 것)
        const { data, error } = await supabase
          .from("payment")
          .select("transaction_key, status, end_at")
          .eq("status", "Paid")
        ...
        if (error) {
          ...
          // 에러가 발생해도 계속 진행 (구독 정보가 없을 수도 있음)
          return;
        }

        if (data) {
          // end_at이 현재 시간보다 미래인 경우에만 구독 중으로 간주
          const endAt = new Date(data.end_at);
          const now = new Date();

          if (endAt > now) {
            setTransactionKey(data.transaction_key);
            setUser((prev) => ({
              ...prev,
              subscriptionStatus: "subscribed",
            }));
  ...
  const onCancelSubscription = () => {
    if (confirm("구독을 취소하시겠습니까?")) {
      ...
      // 훅에서 성공 시 알림 메시지와 페이지 이동을 처리함
      handleCancelSubscription(transactionKey);
    }
  };
